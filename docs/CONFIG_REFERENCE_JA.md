...
| `observe_interval_seconds` | `3600` | エージェントのバックグラウンド監視（`observe`）の実行間隔を秒単位で決定します。10秒以上の値を指定する必要があります（デフォルト: 3600秒 = 1時間）。X (Twitter) API のレートリミットおよび接続切断エラーを回避するため、本番環境では3600秒以上の設定を強く推奨します。 |
...
| **`PHRONEL_OBSERVE_INTERVAL_SECONDS`** | `3600` | **ハイブリッド**: バックグラウンド監視（`observe`）の実行間隔秒数。10秒以上の値が必要（DB設定優先）。 |
...
## 6. ペルソナ & ナレッジ連関表 (`PersonaSourceLink`)
...
## 7. X (Twitter) API レートリミットと接続遮断（Connection Reset）に関する注意事項

X.com (Twitter) APIは、データの読み込み（キーワード検索）および書き込み（ツイート、いいね、フォロー）の双方に対して非常に厳格なレートリミット（回数制限）を課しています。これを無視して高頻度の自動巡回を行うと、アカウントの制限や、API接続自体を物理的に遮断されるペナルティが発生します。

### 安全な運用のためのガイドライン：
* **推奨監視間隔**: 本番運用では `observe_interval_seconds` を必ず **`3600`秒 (1時間) 以上** に設定してください。
  * 巡回ループを5分未満などの超短期間に設定すると、API呼び出し回数が `(巡回回数) × (登録キーワード数)` で掛け算的に増大し、Basicプランの上限である「15分間に100リクエスト」を一瞬で超過して `HTTP 429 Too Many Requests` エラーが発生します。
* **Connection Reset by Peer（強制切断）とアカウントロック**:
  * APIエラー（429超過や、403権限不足）が出ている状態でプログラムが諦めずに数分おきにリクエストを送り続けると、X APIの前段にあるセキュリティシステム（Cloudflare等のWAF）からスパム攻撃と判定され、IPアドレス単位でポート接続を強制切断されるようになります。これが `Connection reset by peer` エラーの正体です。
  * このエラーが発生した場合は、**直ちにエージェントシステムを停止し、最低でも1〜2時間（できれば24時間）アクセスを行わずに「クールダウン」させてください。** 執拗にAPIリクエストを再試行し続けると、開発者ポータルアプリの強制停止やXアカウントの永久凍結に繋がります。
* **書き込み権限（Read and Write）の適用**:
  * 開発者ポータルでアプリの権限設定を「Read」から「Read and Write」に変更した際、必ず **「Access TokenとSecretの再生成（Regenerate）」** を行ってください。古いトークンのままでは書き込み操作（ツイートやいいね）時に `You are not permitted...` エラーが発生し続けます。
* **検索キーワードを厳選する**:
  * 監視対象のキーワードは、ノイズが少なく確度の高い **3〜5個程度** に厳選してください。キーワードが多いほど、1回の自動巡回ごとのAPIリクエスト消費がキーワード倍で加速します。
